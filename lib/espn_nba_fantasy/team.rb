...
module ESPNNBAFantasy
  class Team
    ...
    def trade_players(to_trade = [], to_receive = [], other_team_name)
      ...
      if own_players.include?(nil) || other_players.include?(nil)
        return 'This trade included players not one of the specified teams'
      end

      { 'Trading:' => own_players.map(&:full_name),
        'Receiving:' => other_players.map(&:full_name) }.merge(new_team_stats(own_players, other_players))
    end
