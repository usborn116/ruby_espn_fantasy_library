...
require 'rest-client'
require_relative '../lib/espn_nba_fantasy'
...
describe ESPNNBAFantasy::Team do
  before do
    @league = ESPNNBAFantasy::League.new(780_758_162, 2023, s2, sw)
    @team = @league.teams[5]
  end
  ...
  describe '#initialize' do
    ...
    it 'has team stats' do
      expect(@team.teamstats.keys).to eql(['PTS', 'BLK', 'STL', 'AST', 'OREB', 'DREB', 'TO', 'FGM', 'FTM', '3PTM', 'FGA',
                                           '3PTA', 'FTA', 'AFG%', 'A/TO', 'FT%'])
    end
  ...
  describe '#trade_players' do
    ...
    it 'returns new stats' do
      expect(@result['After:'].keys).to eql(['PTS', 'BLK', 'STL', 'AST', 'OREB', 'DREB', 'TO', 'FGM', 'FTM', '3PTM', 'FGA',
                                             '3PTA', 'FTA', 'AFG%', 'A/TO', 'FT%'])
    end

    it 'returns stat differences' do
      expect(@result['Changes:'].keys).to eql(['PTS', 'BLK', 'STL', 'AST', 'OREB', 'DREB', 'TO', 'FGM', 'FTM', '3PTM', 'FGA',
                                               '3PTA', 'FTA', 'AFG%', 'A/TO', 'FT%'])
    end
  end
